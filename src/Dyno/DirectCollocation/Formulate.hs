...
import Dyno.View.Function ( (:*:)(..), MXFun, toMXFun, callMXFun )
...
makeCollNlp ocp = do
  ...
  dynFun <- toMXFun (dynConstraints cijs taus (ocpDae ocp))
  dynAndCovFun <- toMXFun (propogateCovariance dynFun)

  return $ Nlp { nlpFG = getFg taus ocp dynAndCovFun
               , nlpBX = cat (getBx ocp)
               , nlpBG = cat (getBg ocp)
               , nlpX0 = jfill 0
               , nlpP = cat JNone
...
getFg ::
  ...
  -> MXFun
      ...
      (J (Cov s) :*: J (CollDynConstraint deg r) :*: J (JVec deg o) :*: J x)
  -> (J (CollTraj x z u p s n deg) MX, J JNone MX)
  -> (J S MX, J (CollOcpConstraints n deg x r c h sh sc) MX)
getFg taus ocp dynAndCovFun (collTraj, _) = (obj, cat g)
  where
    ...
    (covs, dcs, outputs, interpolatedXs) = TV.tvunzip4 blah_
    ...
    ff :: J (Cov s) MX -> (CollStage x z u deg MX, J (JVec deg S) MX, J (Cov s) MX) ->
          (J (Cov s) MX, (J (Cov s) MX, J (CollDynConstraint deg r) MX, J (JVec deg o) MX, J x MX))
    ff cov0 (CollStage x0' xzus, stageTimes, covInj) = (cov1, (cov0, dc, output, interpolatedX'))
      where
        cov1 :*: dc :*: output :*: interpolatedX' =
          callMXFun dynAndCovFun (cov0 :*: h :*: parm :*: stageTimes :*: x0' :*: xzs :*: us :*: covInj)
...
propogateCovariance ::
  forall x z u p o s r deg . (Dim deg, View x, View z, View u, View p, View r, View o, View s)
  => MXFun (J S :*: J p :*: J (JVec deg S) :*: J (CollStage x z u deg))
           (J (CollDynConstraint deg r) :*: J (JVec deg o) :*: J x)
  -> (J (Cov s) :*: J S :*: J p :*: J (JVec deg S) :*: J x :*: J (JVec deg (JTuple x z)) :*: J (JVec deg u) :*: J (Cov s)) MX
  -> (J (Cov s) :*: J (CollDynConstraint deg r) :*: J (JVec deg o) :*: J x) MX
propogateCovariance dynFun (p0' :*: h :*: parm :*: stageTimes :*: x0' :*: xzs' :*: us :*: q0') =
    (p1 :*: dynConstrs' :*: outputs :*: interpolatedX)
  where
    collStage :: CollStage x z u deg MX
    collStage = CollStage x0' (cat $ JVec $ TV.tvzipWith catXzu (unJVec (split xzs')) (unJVec (split us)))
    ...
    dynConstrs' :*: outputs :*: interpolatedX = callMXFun dynFun (h :*: parm :*: stageTimes :*: (cat collStage))
    q0 = toMatrix' q0'
    p0 = toMatrix' p0'
